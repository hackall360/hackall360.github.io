...
var prefersReducedMotion2 = window.matchMedia("(prefers-reduced-motion: reduce)");
var setFinalState = (element) => {
  const target = element;
  target.style.opacity = "1";
  target.style.transform = "none";
};
var revealElement = (element, index = 0, fallbackStagger = 0.08) => {
  const target = element;
...
var initGroupedReveals = () => {
  ...
  groups.forEach((group) => {
    ...
    const stagger = Number(group.dataset.motionStagger ?? "0.08");
    inView(
      group,
      () => {
...
prefersReducedMotion2.addEventListener("change", () => {
  if (prefersReducedMotion2.matches) {
    document.querySelectorAll("[data-motion-reveal], [data-motion-child]").forEach(setFinalState);
  }
});

// src/scripts/contact-actions.ts
var COPY_ATTRIBUTE = "data-copy";
var SUCCESS_CLASS = "copy-success";
var DEFAULT_CLASS = "copy-default";
var ACTIVE_BORDER_CLASS = "border-accent";
var SUCCESS_TIMEOUT = 2e3;
var resetButtonState = (button) => {
  const defaultLabel = button.querySelector(`.${DEFAULT_CLASS}`);
  const successLabel = button.querySelector(`.${SUCCESS_CLASS}`);
...
var enhanceCopyButtons = () => {
  const buttons = Array.from(
    document.querySelectorAll(`[${COPY_ATTRIBUTE}]`)
  );
